...
function webhookLibrary.createMessage(properties)
	...
	local EmbedIndex = 0
	function webhookFunctions.addEmbed(title: string, color: number, description: string, thumbnail: string)
		assert(title, "title required")
		assert(description, "description required")
		assert(color, "color required")
		assert(thumbnail, "thumbnail required")
		...
		table.insert(requestTable.Body.embeds, {
			...
			["footer"] = {
				["text"] = "Developed By Someone Cool ;)",
			},
			["fields"] = {},
		})
		local embedFunctions = {}
		function embedFunctions.addField(name, inline, value)
